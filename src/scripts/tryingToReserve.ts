...
async function doTheScript() {
    ...
    let [pubk, priv]: [string, string] = ["", ""]
    {
        ...
        console.log()
        console.log(`Have Passphrase: ${passPhrase}`);
        // eg eyJhbGciOi...
        console.log()
    ...
    if (reserveResult.thingsThatAlreadyExist.length > 0) {
        console.log("The following properties already exist and won't be reserved again:", reserveResult.thingsThatAlreadyExist.map(item => oct.cubeToUrlString(item.cube)[0]))
    }
    ...
    console.log("Ready to reserve the following properties:", reserveResult.thingsToActuallyReserve.map(cube => oct.cubeToUrlString(cube)[0]))

    const groupTextParameters: oct.GroupTextParameters = {
        grp: "CourtyardAndFloor_group_j9xK3mP8wL2z", // randomness
        ali: "localhost",
        p: 3010,
    }
    ...
    if (yn.toLowerCase() !== "y") {
        ...
        for (const cube of reserveResult.thingsToActuallyReserve) {
            const [cubeStr, err] = oct.cubeToUrlString(cube)
            if (err) {
            ...
            let res = await names.sendNameserviceCommand("exists", cubeUrlVr, { pubk, priv })
            ...
            res = await names.sendNameserviceCommand("reserve " + cubeUrlVr + " " + bigKnotfreeToken, cubeUrlVr, { pubk, priv })
            ...
            if ( cube.whichParent === undefined) {
                ...
                res = await names.sendNameserviceCommand(`set option txt meta_group_id ${gtpString}`, cubeUrlVr, { pubk, priv })
