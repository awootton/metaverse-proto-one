...
// then execute this script with: npx ts-node src/scripts/buildingTestMain/reserveSmallPlots.ts

const property1 = "testmain-2n0u5w2p" // It's a small 4 by 4
// and the one to the east of it:
const property2 = "testmain-2n0u4w2p" // needs a duck. lol. See the groupTextParameters below. Really. Seriously. There's a duck in localhost:3010
...
async function doTheWork() {
    ...
    const errReserveVr = await reserveVrFunctions.ReserveVr([property1, property2], groupTextParameters)
    if (errReserveVr) {
