...
// Now we extend that same API with iFrames. This is being executed in PubSubTopicAndSubscribers. This file will all go away soon. We salute you. 

/*  To do this we need 4 message passaging routines. I find it confusing. I also call the 'parent' the mainland and the
 'contentWindow', or iFrame, or children, the island. 

 Note that that we consider ONE layer only. No nested iframes are handled in this scenario. 

    The first two (on the island)
        window.parent.postMessage(msg, "*");                        to the mainland, the 'parent'.
    ...
    On the mainland.
        ...
        window.addEventListener("message", handlePaleMessages(...)    Things going to the island, from BigCity.

    At, AI Gemini, they say: Use the window.postMessage() API:
    ...
    RPC_Gadget has been build with this.

    // Our seniario one.

    // 1) 'Orange' (TheFormallyOrange4WestApp, aka testmain-2n0u4w2p in WorldsTest1) subscribes to an island command set, 
    //     with orange stuff, ideas, and commands in it. Nothing really. topic: testmain-2n0u4w2p-island-centre
    //     It's 2 north and 4 west of the 16 meter places.
...
// Delete this soon because I'm tired of seeing it in the global search.
...
//         key: key,
//         who: who,
//         cmd: "execute_theSub_scribe",
//         magic: messes.magicMessageNumber,
//     }
//     tracebridgemesses('PubSubBridge execute_theSub_scribe send to window.parent.postMessage', submsg)
...
//         var msg: messes.MessagePublishClass = {
//             to: key,
...
// // Fetch him a cab.
// window.addEventListener("message", handleCountryMessages, false);
...
// function handleCountryMessages(event: MessageEvent) { // this guy just got back.
...
//     }
//     tracebridgemesses('PubSubBridge handleCountryMessages', event)
...
//         if (baseMessage.cmd === "execute_thePu_blish") {
//             // we have a valid message. 
//             const publishCommand = baseMessage as messes.MessagePublishClass
...
//         if (baseMessage.cmd === "execute_theSub_scribe") {
//             // we have a valid message. 
...
//             return
//         }
//         console.log("PubSubBridge handleCountryMessages: unknown command", baseMessage.cmd)
//     }
// }

// // Ready for a break from the hard city life. This message can't wait to get away.

// window.addEventListener("message", handleBigCityMessages, false);
...
// export function handleBigCityMessages(event: MessageEvent) {
//     tracebridgemesses('PubSubBridge handleBigCityMessages', event)
//     // anything might come throuhe here. We have to be agressibe about filtering out the messages that are not for us.
//     if (AmIOnTheMainland()) {
//         return; // it just came from the mainland. It's BigCity.
//     }
//     // Implement the handler for BigCity messages here
...
//         if (baseMessage.cmd === "execute_a_Pub_lish") {
//             // we have a valid message. 
//             const publishCommand = baseMessage as messes.MessagePublishClass
//             // we want to call the callback that was registered for this key.
//             // and we shall. 
...
//             return
//         }
//         if (baseMessage.cmd === "execute_a_Sub_scribe") {
//             // we have a valid message. 
//             const publishCommand = baseMessage as messes.MessagePublishClass
//             // we want to call the callback that was registered for this key.
//             // and we shall. 
...
//             return
//         }
//         console.log("PubSubBridge handleBigCityMessages: unknown command", baseMessage.cmd)
//     }
// }
